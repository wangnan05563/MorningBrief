...
test.describe('频道管理', () => {
  ...
  test('列表加载显示频道', async ({ page }) => {
    ...
    await expect(page.getByText('频道管理').first()).toBeVisible({ timeout: 15000 })
    // 表格应展示 mock 数据
    await expect(page.getByText('科技频道')).toBeVisible({ timeout: 15000 })
    await expect(page.getByText('财经频道')).toBeVisible({ timeout: 15000 })
  })
  ...
  test('admin 显示新增按钮', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: '新增频道' })).toBeVisible({ timeout: 15000 })
  })
  ...
  test('operator 无新增与操作按钮', async ({ page }) => {
    ...
    // operator 不应看到新增按钮
    await expect(page.getByRole('button', { name: '新增频道' })).toHaveCount(0, { timeout: 15000 })
    // 也不应看到编辑/删除按钮
    await expect(page.getByRole('button', { name: '编辑' })).toHaveCount(0, { timeout: 15000 })
    await expect(page.getByRole('button', { name: '删除' })).toHaveCount(0, { timeout: 15000 })
  })
  ...
  test('新增频道表单校验', async ({ page }) => {
    ...
    // 弹窗显示
    await expect(page.locator('.el-dialog').filter({ hasText: '新增频道' })).toBeVisible({ timeout: 15000 })

    // 直接点确认，触发 required 校验
    await page.getByRole('button', { name: '确认' }).click()
    // Element Plus 表单校验提示
    await expect(page.getByText('请输入频道名称')).toBeVisible({ timeout: 15000 })
  })
  ...
  test('新增频道成功', async ({ page }) => {
    ...
    // 成功提示（后端代码实际消息为"已创建"）
    await expect(page.locator('.el-message').getByText('已创建')).toBeVisible({ timeout: 15000 })
  })
  ...
  test('编辑频道预填数据', async ({ page }) => {
    ...
    // 弹窗标题应为"编辑频道"
    await expect(page.locator('.el-dialog').filter({ hasText: '编辑频道' })).toBeVisible({ timeout: 15000 })
    // 名称应预填"科技频道"
    await expect(page.getByPlaceholder('请输入频道名称')).toHaveValue('科技频道', { timeout: 15000 })
  })
  ...
  test('删除频道二次确认', async ({ page }) => {
    ...
    await expect(page.locator('.el-message-box').getByText(/确认删除频道/)).toBeVisible({ timeout: 15000 })
    // 确认按钮文本为"确认删除"（handleDelete 中 confirmButtonText 指定）
    await page.locator('.el-message-box').getByRole('button', { name: '确认删除' }).click()

    // 成功提示（后端代码实际消息为"已删除"）
    await expect(page.locator('.el-message').getByText('已删除')).toBeVisible({ timeout: 15000 })
  })
  ...
  test('启用/停用切换', async ({ page }) => {
    ...
    await expect(page.locator('.el-message').getByText(/已禁用|已启用/).first()).toBeVisible({ timeout: 15000 })
  })
})
