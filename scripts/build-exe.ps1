<#
.SYNOPSIS
    20_News EXE 构建脚本（PyInstaller + Inno Setup）
.DESCRIPTION
...
$buildReadyMark = "$buildVenv\.20news-build-ready"
$distDir        = "dist\20-news"
$frontendDir    = "admin-web"
$specFile       = "20-news.spec"
$backendDir     = "backend"
...
function Write-Step { param([string]$Msg) Write-Host "[20News-Build] $Msg" -ForegroundColor Cyan }
function Write-OK   { param([string]$Msg) Write-Host "[20News-Build]   [OK] $Msg" -ForegroundColor Green }
function Write-Warn { param([string]$Msg) Write-Host "[20News-Build]   [WARN] $Msg" -ForegroundColor Yellow }
function Write-Err  { param([string]$Msg) Write-Host "[20News-Build]   [FAIL] $Msg" -ForegroundColor Red }
...
Write-Host "  20_News EXE Build" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
...
AppPublisher=20_News
DefaultDirName={autopf}\20_News
DefaultGroupName=20_News
UninstallDisplayIcon={app}\20-news.exe
OutputDir=dist
OutputBaseFilename=20News-Setup-v{#MyAppVersion}
Compression=lzma2
...
[Files]
; 打包 dist/20-news/ 下所有文件（排除日志和数据）
Source: "dist\20-news\*"; DestDir: "{app}"; Excludes: "*.log,logs\*,data\*"; Flags: ignoreversion recursesubdirs createallsubdirs
[Icons]
Name: "{group}\20_News"; Filename: "{app}\20-news.exe"
Name: "{commondesktop}\20_News"; Filename: "{app}\20-news.exe"; Tasks: desktopicon
[Run]
Filename: "{app}\20-news.exe"; Description: "启动 20_News"; Flags: nowait postinstall skipifsilent
"@
    ...
    if (-not $iscc) {
        ...
        if ($LASTEXITCODE -ne 0) {
            ...
            $setupExe = "dist\20News-Setup-v$version.exe"
...
Write-Host "  EXE:      $distDir\20-news.exe"
Write-Host ""
Write-Host "  使用方式:" -ForegroundColor Cyan
Write-Host "    1. 编辑 dist\20-news\.env 填入实际密钥（JWT_SECRET / LLM_API_KEY / COS_* 等）"
