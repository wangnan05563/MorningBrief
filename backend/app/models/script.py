...
from sqlalchemy import BigInteger, String, Integer, Text, DateTime, Date, JSON, Enum as SAEnum, Index
...
class Script(Base):
    ...
    segments: Mapped[dict] = mapped_column(
        JSON, nullable=False, comment="分段结构 [{seq,title,content,start_sec,end_sec,material_ids}]"
    )
    ...
    referenced_materials: Mapped[Optional[list]] = mapped_column(JSON, comment="引用素材 URL 列表")
    categories: Mapped[Optional[list]] = mapped_column(JSON)
    status: Mapped[Optional[ScriptStatus]] = mapped_column(
        SAEnum(ScriptStatus), default=ScriptStatus.draft
    )
