...
# cloudflared 凭证目录与文件名常量：避免字面量重复触发 S1192
_CLOUDFLARED_DIR = ".cloudflared"
_CLOUDFLARED_CERT = "cert.pem"


class BinaryDownloadError(RuntimeError):
    """二进制下载失败，附带手动放置指引。

    前端读取 manual_path 和 download_urls 渲染手动放置步骤。
    """
...
class TunnelProvider(ABC):
    ...
    def __init__(self, local_port: int, binary_path: str = ""):
        ...
        # 用户手动放置的二进制路径（可选，优先于自动下载，便于离线环境）
    ...
    def stop(self) -> None:
        ...
        if self._process:
            ...
            self._process = None
        self._public_url = None
        logger.info("[%s] 隧道已关闭", self.binary_name)
    ...
    def _ensure_binary(self) -> Path:
        ...
        if self._manual_binary_path:
            ...
            if manual.exists():
                logger.info("[%s] 使用手动放置的二进制: %s", self.binary_name, manual)
                return manual
            logger.warning(
                "[%s] 配置的 binary_path 不存在: %s，回退到自动下载",
                self.binary_name, manual,
            )

        # V1.2 约束：可写数据放 data 目录（exe 同级）
        data_dir = resolve_data_dir()
        binary_path = data_dir / self.binary_name

        if not binary_path.exists():
            logger.info("[%s] 二进制不存在，开始下载...", self.binary_name)
    ...
    def _download_binary(self, target: Path) -> None:
        ...
        for url in self.download_urls:
            try:
                logger.info("[%s] 尝试下载源: %s", self.binary_name, url)
                self._do_download(url, target)
                logger.info("[%s] 下载完成: %s", self.binary_name, target)
                return
            except Exception as e:
                logger.warning("[%s] 下载源失败 %s: %s", self.binary_name, url, e)
                last_error = e

        # 全部失败：抛出含指引的异常，前端可渲染手动放置步骤
        raise BinaryDownloadError(
            f"{self.binary_name} 所有下载源均失败: {last_error}",
            manual_path=str(resolve_data_dir() / self.binary_name),
    ...
    def _wait_for_url(self, url_pattern: re.Pattern, timeout: int = 15) -> str:  # NOSONAR
    ...
    def _start_process(self, cmd: list[str], url_pattern: re.Pattern, timeout: int = 15) -> str:
        ...
        logger.info("[%s] 隧道已建立: %s", self.binary_name, url)
        return url
...
class CloudflareProvider(TunnelProvider):
    ...
    def _find_cert_pem_paths(self) -> list[Path]:
        ...
        - 从 login 输出中提取的路径（兜底）
        """
        paths: list[Path] = [Path.home() / _CLOUDFLARED_DIR / _CLOUDFLARED_CERT]
        local_app_data = os.environ.get("LOCALAPPDATA")
        if local_app_data:
            paths.append(Path(local_app_data) / _CLOUDFLARED_DIR / _CLOUDFLARED_CERT)
        app_data = os.environ.get("APPDATA")
        if app_data:
            paths.append(Path(app_data) / _CLOUDFLARED_DIR / _CLOUDFLARED_CERT)
    ...
    def start_login(self) -> dict:  # NOSONAR
...
class CpolarProvider(TunnelProvider):
    ...
    def _configure_authtoken(self, binary: Path) -> None:
        """首次启动前配置 authtoken（写入 cpolar 配置文件，幂等操作）。"""
...
class TailscaleProvider(TunnelProvider):
    ...
    def __init__(self, local_port: int, binary_path: str = "", path_prefix: str = ""):
        ...
        # 路径区分模式下 stop() 保留 Funnel 配置，需用此标志位避免 status 重新查询 CLI 后覆盖停止状态
        self._stopped = False
    ...
    def _run_cli(self, *args: str, timeout: int = 20) -> subprocess.CompletedProcess:
        ...
        # 中文 Windows 默认用 GBK 解码会导致含非 ASCII 字符（如节点用户名）的 JSON 解析失败
        result = subprocess.run(
    ...
    def _funnel_url_from_status(self, data: dict) -> str | None:  # NOSONAR S3776: funnel 多状态解析逻辑不可再拆分
        """从 funnel status --json 提取当前应用的公网 URL。

        多应用路径模式下，URL 包含 path_prefix（如 https://host/news/）；
    ...
    def status(self) -> str:
        if self._binary_path is None:
            return "stopped"
        # 用户主动停止后直接返回 stopped，避免重新查询 CLI 把 _public_url 又赋回去
        # （路径区分模式下 stop() 保留 Funnel 配置，CLI 查询仍会返回 URL）
        if self._stopped:
            return "stopped"
        try:
    ...
    def start(self) -> str:  # NOSONAR
        ...
        # 显式指定 UTF-8 解码：与 _run_cli 保持一致，避免中文 Windows 默认 GBK 解码失败
    ...
    def stop(self) -> None:
        # 标记为已停止：status 属性据此返回 stopped，避免路径区分模式下重新查询 CLI 覆盖状态
        self._stopped = True
        ...
        if self._path_prefix:
            self._public_url = None
            logger.info("[tailscale] 路径区分模式：保留 Funnel 配置 %s", self._path_prefix)
            return
...
def create_provider(provider_name: str, local_port: int, **kwargs) -> TunnelProvider:
